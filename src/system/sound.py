...
class SoundMixer:
    ...
    def add_music_channel(self, music: SoundInstance) -> None:
        if music.sound not in self.music_channels:
            ...
            self.music_channels[music.sound] = channel
            self.music_sounds_pos[music.sound] = 0
            self.last_music_channel += 1

            channel.set_endevent(SoundEvents.MUSIC_END_EVENT)
            channel.play(self.sounds[music.sound], -1)
            channel.pause()


    def play_music(self, music: SoundInstance) -> None:
        if self.current_music is None:
            self.current_music = music.sound
            self.music_channels[self.current_music].unpause()
            return

        if self.current_music != music.sound:
            self.next_music = music.sound
