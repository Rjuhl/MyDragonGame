...
def game_loop_callback(context: PageContext) -> None:
    ...
    SoundMixer().play_music(Sound.GAME_TRACK_1)
...
def settings_callback(context: PageContext) -> None:
    ...
    SoundMixer().play_music(Sound.MAIN_TRACK)
...
def previous_page_callback(context: PageContext) -> None:
    ...
    if "prev_page" in context:
        ...
        # Will need to update as music is added
        sound = Sound.GAME_TRACK_1 if context["next_page"] == GamePage.__name__ else Sound.MAIN_TRACK
        SoundMixer().play_music(sound)